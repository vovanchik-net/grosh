...
#if defined(NDEBUG)
# error "Grosh cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Grosh Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s Grosh additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nPowHeight, const Consensus::Params& consensusParams)
{
    if (consensusParams.forkNumber(nPowHeight) >= 3)
        return ((29 - std::min(nPowHeight >> 16, 28)) * COIN / 2);
...
std::string scriptToStr (const CScript& script) {
    std::string s;
    CTxDestination addr;
    if (ExtractDestination(script, addr)) { s = EncodeDestination(addr); } else { s = ScriptToAsmStr(script); }
    return s;
}
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (fTxIndex) {
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                addressKeyValue.push_back(std::make_pair(CAddressKey(scriptToStr(out.scriptPubKey), COutPoint(hash, k)), 
                    CAddressValue(0, 0)));
            }
        }
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fTxIndex) {
                    ...
                    addressKeyValue.push_back(std::make_pair(CAddressKey(scriptToStr(coin.out.scriptPubKey), out), 
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (fTxIndex) {
            for (size_t j = 0; j < tx.vin.size(); j++) {
                ...
                addressKeyValue.push_back(
                    std::make_pair(CAddressKey(scriptToStr(coin.out.scriptPubKey), tx.vin[j].prevout), addrval));
            }
        }
        ...
        if (fTxIndex) {
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                const CTxOut &out = tx.vout[k];
                if (out.scriptPubKey.IsUnspendable()) continue;
                addressKeyValue.push_back(std::make_pair(CAddressKey(scriptToStr(out.scriptPubKey), COutPoint(tx.GetHash(), k)),
