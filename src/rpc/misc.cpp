...
bool getAddressesFromParams(const UniValue& params, std::vector<std::string> &addresses) {
    if (params[0].isStr()) {
        if (!IsValidDestination(DecodeDestination(params[0].get_str()))) return false;
        addresses.push_back(params[0].get_str());
    } else if (params[0].isObject()) {
        UniValue addressValues = find_value(params[0].get_obj(), "addresses");
        if (!addressValues.isArray()) return false;
        std::vector<UniValue> values = addressValues.getValues();
        for (auto it : values) {
            if (!IsValidDestination(DecodeDestination(it.get_str()))) return false;
            addresses.push_back(it.get_str());
        }
    } else {
        return false;
    }
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    for (auto it : info) {
        ...
        output.pushKV("address", it.first.addr);
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "grosh-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\ngrosh-cli has the option -getinfo to collect and format these in the old format."
    );
}
