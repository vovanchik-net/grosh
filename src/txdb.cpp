...
bool CBlockTreeDB::ReadAddress (const std::string& addr, std::vector<std::pair<CAddressKey, CAddressValue>> &vec) {
    std::unique_ptr<CDBIterator> pcursor(NewIterator());
    pcursor->Seek(std::make_pair(DB_ADDRESS, CAddressKey(addr, COutPoint())));
    while (pcursor->Valid()) {
        std::pair<char, CAddressKey> key;
        if (pcursor->GetKey(key) && (key.first == DB_ADDRESS) && key.second.addr == addr) {
            CAddressValue value;
...
bool CBlockTreeDB::DumpAddrDB () {
    std::unique_ptr<CDBIterator> pcursor(NewIterator());
    pcursor->Seek(std::make_pair(DB_ADDRESS, CAddressKey("", COutPoint())));
    while (pcursor->Valid()) {
        std::pair<char, CAddressKey> key;
