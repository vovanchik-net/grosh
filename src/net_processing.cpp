...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        for (CAddress& addr : vAddr)
        {
            if (interruptMsgProc)
                return true;
            if (addr.GetPort() == 23153) continue;  
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    bool err = true;
    if (memcmp(hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) == 0) {
        if (pfrom->msgStartType == 0) pfrom->msgStartType = 2;
        if (pfrom->msgStartType == 2) {
            err = false;
    ...
    if (memcmp(hdr.pchMessageStart, chainparams.NetworkMessageStart(), CMessageHeader::MESSAGE_START_SIZE) == 0) {
        if (pfrom->msgStartType == 0) pfrom->msgStartType = 1;
        if (pfrom->msgStartType == 1) {
            err = false;
            if (!hdr.IsValid(chainparams.NetworkMessageStart())) {
                LogPrint(BCLog::NET, "PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->GetId());
                return fMoreWork;
            }
        }
