...
uint32_t GetNextWorkRequired(const CBlockIndex *pindexLast, const CBlockHeader *pblock, const Consensus::Params &params) {
    ...
    int64_t srcTimes = 0;
    int64_t nTimes = 0;
    int TargetSpacing = params.nPowTargetSpacingBegin / 4;
    if (!fProofOfStake && (pindexLast->nHeight < params.newProofHeight)) return bnLimit.GetCompact();
    if (fProofOfStake && (pindexLast->nHeight < params.newProofHeight + 7168)) bnLimit >>= 8;
