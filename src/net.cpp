...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    if (pnode->msgStartType == 0) {
        if (Params().NetworkMessageStart()[0] == 0) {
            pnode->msgStartType = 2;
        } else { 
            pnode->msgStartType = 1; 
        }
    }
    CMessageHeader::MessageStartChars msghdr;
    if (pnode->msgStartType == 1) {
        memcpy(msghdr, Params().NetworkMessageStart(), CMessageHeader::MESSAGE_START_SIZE);
    } else {
        memcpy(msghdr, Params().MessageStart(), CMessageHeader::MESSAGE_START_SIZE);
    }
    CMessageHeader hdr(msghdr, msg.command.c_str(), nMessageSize);
